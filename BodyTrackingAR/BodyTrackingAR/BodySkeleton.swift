...
class BodySkeleton: Entity {
    ...
    required init(for bodyAnchor: ARBodyAnchor) {
        ...
        for jointName in ARSkeletonDefinition.defaultBody3D.jointNames {
            ...
            // Create an entity for the joint, add to joints dictionary, and add it to the parent entity
        ...
        for jointName in ARSkeletonDefinition.defaultBody3D.jointNames {
            if let jointEntity = joints[jointName],
               let jointEntityTransform = bodyAnchor.skeleton.modelTransform(
                ...
                jointEntity.position = jointEntityOffsetFromRoot + rootPosition  // related to world reference frame
    ...
    private func createSkeletonBone(bone: Bones, bodyAnchor: ARBodyAnchor) -> SkeletonBone? {
        ...
        let jointFromEntityOffsetFromRoot = simd_make_float3(
            fromJointEntityTransform.columns.3
        ) // realative to root (i.e. hipJoint)

        let jointFromEntityPossition = jointFromEntityOffsetFromRoot + rootPosition // relative to world reference frame

        let jointToEntityOffsetFromRoot = simd_make_float3(
            toJointEntityTransform.columns.3
        ) // realative to root (i.e. hipJoint)

        let jointToEntityPossition = jointToEntityOffsetFromRoot + rootPosition // relative to world reference frame

        let fromJoint = SkeletonJoint(name: bone.jointFromName, position: jointFromEntityPossition)

        let toJoint = SkeletonJoint(name: bone.jointFromName, position: jointToEntityPossition)
